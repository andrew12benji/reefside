// audio.ts — Reef Symphony Audio Engine (Web Audio API)
let _ctx: AudioContext | null = null;
let _master: GainNode | null = null;
let _enabled = false;
let _ambienceTimer: ReturnType<typeof setTimeout> | null = null;
...
export function playNetEntangle(): void {
...
export function playPearl(): void {
...
// ── AMBIENT SOUNDSCAPE ───────────────────────────────────────────────────────
const PENTATONIC = [
  130, 146, 164, 196, 220, 261, 294, 329, 392, 440, 523, 587, 659, 784, 880,
];

export function updateAmbience(reefHealth: number): void {
  ...
  _ambienceTimer = setTimeout(() => {
    ...
    if (reefHealth > 70) {
      ...
      if (Math.random() > 0.5)
        setTimeout(() => beep(root * 3, 0.4, "sine", 0.01), 200);
      if (Math.random() > 0.7)
        setTimeout(() => beep(root * 1.5, 0.5, "triangle", 0.012), 350);
      ...
      if (Math.random() > 0.6)
        setTimeout(() => beep(f * 1.07, 1.5, "sawtooth", 0.007), 700);
    }
  }, delay);
}
